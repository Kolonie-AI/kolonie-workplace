...
describe('kanban board — the six fixed Colony lanes', () => {
  ...
  it('offers no way to add, rename, reorder or delete a lane', async () => {
    ...
    expect(within(board).queryByRole('button', { name: /add lane/i })).toBeNull()
    expect(within(board).queryByRole('button', { name: /new (lane|bucket|column)/i })).toBeNull()
    expect(within(board).queryByRole('button', { name: /rename/i })).toBeNull()
    expect(within(board).queryByRole('button', { name: /delete/i })).toBeNull()
    expect(within(board).queryByRole('button', { name: /create/i })).toBeNull()
  ...
  it('offers no Add another list control and no list menu that mutates a lane', async () => {
    ...
    const board = screen.getByTestId('kanban-board')
    expect(within(board).queryByRole('button', { name: /add (another )?list/i })).toBeNull()
    expect(within(board).queryByRole('button', { name: /rename|move list|copy list|archive list|delete list/i })).toBeNull()
